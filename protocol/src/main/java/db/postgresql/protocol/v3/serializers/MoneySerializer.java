...
public class MoneySerializer extends Serializer {
    ...
    public MoneySerializer(final Locale locale) {
        super(oids(790), classes(BigDecimal.class));
        this.locale = locale;
    }
    ...
    public Bindable bindable(final BigDecimal bd, final Format format) {
        return new Bindable() {
            ...
            public int getLength() { return MoneySerializer.this.length(bd, format); }
            public void write(final Stream stream) { MoneySerializer.this.write(stream, bd, format); }
        };
    }
}
