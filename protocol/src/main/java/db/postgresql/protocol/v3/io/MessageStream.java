...
public class MessageStream {
    ...
    private void ensureForSend(final int size) {
        if(size > sendBuffer.capacity()) {
            String msg = String.format("MessageStream can only hold %d bytes, " +
    ...
    public MessageStream put(ByteBuffer buffer) {
        ensureForSend(buffer.remaining());
        sendBuffer.put(buffer);
        return this;
    }

    public MessageStream put(byte b) {
        ensureForSend(1);
        sendBuffer.put(b);
        return this;
    }

    public MessageStream put(byte[] bytes) {
        ensureForSend(bytes.length);
        sendBuffer.put(bytes);
        return this;
    }

    public MessageStream putShort(short s) {
        ensureForSend(2);
        sendBuffer.putShort(s);
        return this;
    }

    public MessageStream putInt(int i) {
        ensureForSend(4);
        sendBuffer.putInt(i);
        return this;
    }

    public MessageStream putNull() {
        put((byte) 0);
        return this;
    }
